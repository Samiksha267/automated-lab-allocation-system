...
**Status:** Requirements marked ✅ are implemented and verified as of Phase 15. Requirements marked **(partial)** have their underlying hard-constraint logic, candidate generation, scoring, explanation, and/or alternative-suggestion logic implemented and verified but are not yet reachable through any end-user workflow — PDF import (Phase 19), which is the remaining unbuilt caller for the REGULAR-timetable side of this same pipeline, doesn't exist yet. As of Phase 15, the CR-facing EXTRA-lab search/booking/cancellation workflow (`ExtraLabController`/`ExtraLabService`) is real, production-facing, and verified both by tests and live against the Dockerized stack — every FR below that workflow satisfies is now marked fully ✅, not partial. FCFS concurrency (FR-24) remains explicitly **not** implemented — Phase 16 owns it; Phase 15's booking path is transactionally revalidated but does not yet close the concurrent-request race window (see docs/15-DESIGN-DECISIONS.md).
...
- **FR-24** Extra-lab requests are served First-Come-First-Served among valid requests; FCFS ordering never overrides a hard constraint. *(Phase 16 — not yet implemented. Phase 15's booking path performs fresh, transactional hard-constraint revalidation immediately before persisting, which closes the "stale search result" gap but does **not** yet close the concurrent-simultaneous-request race window — see docs/15-DESIGN-DECISIONS.md's Phase 15/16 boundary ADR.)*
