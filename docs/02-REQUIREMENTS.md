...
- **FR-23** *(partial)* ✅ When a requested slot is invalid, the system provides ranked alternative allocations (different lab, different time, or both) rather than a bare rejection. *(`AlternativeSuggestionService`, Phase 13 — implemented and verified: "different lab, same time" is already resolved by Phase 10-12's own candidate generation (no separate search needed); "different time" is a real, bounded, ranked search reusing the full Phase 10/11/12 pipeline per candidate slot, only attempted when at least one candidate is structurally viable. Not yet reachable through any end-user workflow — the CR-facing search endpoint (Phase 15) that will call it doesn't exist yet. Does not implement "both lab and time" as a combined search dimension beyond what per-slot re-ranking already covers, and never searches across the multi-session scope Phase 14's backtracking will own.)*
