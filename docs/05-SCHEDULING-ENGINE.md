...
Build AllocationRecommendation: recommended candidate + ranked valid alternatives + explained rejections    [ExplainableAllocationService - implemented, Phase 12]
      ↓
(advisory only when called for SEARCH - no persistence, no reservation)
```

**Operationally exposed for the first time in Phase 15** (`ExtraLabController`/`ExtraLabService`, `com.college.laballocation.scheduling.extra`): `POST /api/allocations/extra/search` calls this exact pipeline (via `AlternativeSuggestionService.findAlternatives`, which itself calls `ExplainableAllocationService.recommend`) for a real, authenticated CR request, unmodified. `POST /api/allocations/extra` (booking) re-runs only the final two steps - `SchedulingContextFactory.build` + `ConstraintEngine.evaluate` - against the one selected candidate, immediately before persisting, inside one transaction. This is a single-request revalidation - the concurrency-safe commit-time guarantee against a *simultaneous* competing request is Phase 16's addition (database-level exclusion constraints + a per-division lock, docs/03-SYSTEM-ARCHITECTURE.md §24), layered underneath this same pipeline, not a replacement for it.

**Hard constraints and Phase 16's database concurrency mechanism are complementary, never redundant, never a substitute for each other:** `ConstraintEngine`/HC-01..HC-12 detect *semantic* conflicts against whatever data one query can see at one instant - "is this candidate valid, given the data I just read?" PostgreSQL's exclusion constraints (V11 migration) and the per-division lock instead prevent *race-condition* violations that no single query's semantic correctness could ever guard against - "did some other transaction's data change between my read and my write?" A candidate can be perfectly semantically valid when `ConstraintEngine` checks it and still lose the booking, cleanly, if another transaction's insert commits first; that loss is Phase 16's mechanism working exactly as intended, not a bug in HC-01/02/04/05. See docs/06-CONSTRAINTS.md's per-HC "database concurrency counterpart" notes and docs/03-SYSTEM-ARCHITECTURE.md §24.
