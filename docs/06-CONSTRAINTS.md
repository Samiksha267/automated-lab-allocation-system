...
- **Defense-in-depth, reached in production for the first time in Phase 15:** `ExtraLabService` (`com.college.laballocation.scheduling.extra`) constructs a real `SchedulingActor{userId, CR}` on every `SchedulingRequest` it builds, so HC-11 genuinely runs (and would genuinely fail a mismatched request) even though `ExtraLabService` itself already independently resolves and enforces ownership via `CrOwnershipService` *before* ever building that request. Two independent layers, not one: an API/service-level check (`CrOwnershipService.getCurrentAssignment`/`.requireOwnsDivision`) that fails fast with a clean domain error, and HC-11 inside the constraint pipeline as a structural guarantee that can never be silently bypassed by some future code path that constructs a `SchedulingRequest` without going through `ExtraLabService`'s ownership resolution. Verified live: a CR-supplied foreign-division batch/subject combination is rejected before HC-11 even has a chance to run (via HC-12's own batch/division check or faculty-resolution failing to find any assignment) — HC-11 remains the structural backstop, not the only line of defense.
