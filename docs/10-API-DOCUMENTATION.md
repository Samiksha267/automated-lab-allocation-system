...
|---|---|
| `GET /api/labs/available?subjectId=&date=&startTime=&endTime=` | Subject requirements (Phase 6), allocation persistence (Phase 8), and the constraint engine itself (Phase 9) are all implemented and verified; only candidate generation (Phase 10) and this endpoint's own orchestration are still missing |
...
**Planned, genuinely not yet implemented in any form** (Phase 10+): `NO_VALID_ALLOCATION` (candidate generation finds zero valid candidates — Phase 10), `INVALID_STATE_TRANSITION` (generic allocation-orchestration state errors beyond the specific ones `Allocation`/`ScheduleVersion` already throw — Phase 15+). Every other HC-01..HC-12 violation code is already implemented in code — see the line above.
