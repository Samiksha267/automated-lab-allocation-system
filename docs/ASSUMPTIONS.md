...
### A-36: CR extra-lab booking supports both `BATCH` and `DIVISION` targets, not `BATCH`-only
**Assumption/discovery:** No document anywhere states whether the Phase 15 CR-facing EXTRA workflow should support only `BATCH`-targeted bookings (a single batch's makeup practical - the everyday case) or also `DIVISION`-targeted bookings (a whole-division session, e.g. a makeup guest lecture affecting every batch at once). The general `Allocation`/`SchedulingRequest` domain model has supported both since Phase 8, and nothing in docs/02-REQUIREMENTS.md, docs/09-AUTHORIZATION-RBAC.md, or the CR's described role narrows this specifically for the EXTRA workflow - the phase brief itself flagged this as a real design question, not something to guess through if genuinely ambiguous.
**Resolution:** Both are supported. `ExtraLabSearchRequest`/`ExtraLabBookingRequest` accept a `targetType` field exactly as `SchedulingRequest` already does; `ExtraLabService` passes it through unchanged to the existing pipeline, which already validates the `BATCH`/`DIVISION` structural invariant (`SchedulingRequest`'s own compact constructor) and academic-relationship correctness (HC-12) for both. This was judged a genuine, non-contradictory design decision (nothing in the repository restricts CR bookings to `BATCH` specifically) rather than a stop condition, since choosing the domain model's already-established, already-tested capability is the more conservative choice than arbitrarily narrowing it.
**Impact:** A CR can book a `DIVISION`-wide EXTRA session for their own division (e.g. a whole-division makeup session), not only a single batch's. If this later proves too broad for real CR usage (e.g. a real institutional policy restricts CRs to batch-level bookings only), narrowing `ExtraLabController`'s accepted `targetType` values is a small, backward-compatible follow-up - not a redesign, since the underlying `Allocation`/constraint model already handles both correctly either way.
